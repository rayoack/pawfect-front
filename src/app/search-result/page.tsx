    ...
    return (
        <>
            ...
            <div id="header" className='relative w-full'>
                <MenuPet />
                <Breadcrumb heading='Resultado da Busca' subHeading='Busca' />
            </div>
            <div className="shop-product breadcrumb1 lg:py-20 md:py-14 py-10">
                <div className="container">
                    <div className="heading flex flex-col items-center">
                        <div className="heading4 text-center">Encontrados {filteredData.length} resultados para {String.raw`"`}{query}{String.raw`"`}</div>
                    ...
                    <div className="list-product-block relative md:pt-10 pt-6">
                        <div className="heading6">Produtos encontrados: {query}</div>
                        <div className={`list-product hide-product-sold grid lg:grid-cols-4 sm:grid-cols-3 grid-cols-2 sm:gap-[30px] gap-[20px] mt-5`}>
                            {currentProducts.map((item) => (
                                item.id === 'no-data' ? (
                                    <div key={item.id} className="no-data-product">Nenhum produto encontrado para esta busca.</div>
                                ) : (
